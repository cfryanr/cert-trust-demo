...
//    private static final Logger log = LoggerFactory.getLogger(CloudFoundryCertificateTruster.class);
    ...
    void trustCertificatesInternal() {
        System.out.println("starting trusting certificate ***********************************");
            ...
            try {
                ...
                if ("https".equals(cfTargetUrl.getProtocol()) && host != null) {
                    ...
                    try {
                        ...
                        System.out.println("trusting certificate succeeded for " + host + ":" + httpsPort);
                    } catch (Exception e) {
                        System.out.println("trusting certificate failed for " + host + ":" + httpsPort);
                        e.printStackTrace();
                    }
                }
            } catch (MalformedURLException e1) {
                System.out.println("Cannot parse CF_TARGET '{}' as a URL: " + cfTarget);
            }
        }
            ...
            for (String hostAndPort : trustCerts.split(",")) {
                ...
                if (host != null && host.length() > 0 && port > 0 && port < 65536) {
                    try {
                        ...
                        System.out.println("trusting certificate at succeeded for: " + host + ":" + port);
                    } catch (Exception e) {
                        System.out.println("trusting certificate failed for " + host + ":" + port);
                        e.printStackTrace();
                    }
                }
            }
